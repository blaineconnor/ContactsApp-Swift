...
class Anasayfa: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        _ = vm.kisilerListesi.subscribe(onNext: {liste in
...
extension Anasayfa : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let silAction = UIContextualAction(style: .destructive, title: "Sil"){contextualAction,view,bool in
            ...
            let alert = UIAlertController(title: "Silme İşlemi", message: "\(kisi.kisi_ad!)", preferredStyle: .alert)
            let iptalAction = UIAlertAction(title: "İptal", style: .cancel)
            alert.addAction(iptalAction)
            let evetAction = UIAlertAction(title: "Evet", style: .destructive) {
                action in self.vm.sil(kisi_id: kisi.kisi_id!)
            }
